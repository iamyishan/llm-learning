...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, time_emb_size, qsize, vsize, fsize, cls_emb_size):
        ...
        self.seq1 = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channel),  # 对批次所有样本按每个通道做归一化，不改变输入的尺寸
            nn.ReLU(),
        )
    ...
    def forward(self, x, t_emb, cls_emb):  # t_emb: (batch_size,time_emb_size)
        ...
        t_emb = self.relu(self.time_emb_linear(t_emb)).view(x.size(0), x.size(1), 1,
                                                            1)  # t_emb: (batch_size,out_channel,1,1)
